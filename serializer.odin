...
serialize_tile :: proc(s: ^Serializer, tile:^tile, loc := #caller_location) -> bool {
    serializer_debug_scope(s, "Tile")
    serialize(s, &tile.q, loc)or_return
    serialize(s, &tile.r, loc)or_return
    {
    ...
    {
        context.allocator = context.temp_allocator
        serialize(s, &tile.border, loc)or_return
    }
    serialize(s, &tile.moveable, loc)or_return
    serialize(s, &tile.terrain, loc)or_return    
    return true
}

serialize_world_space :: proc (s:^Serializer, world:^World_Space, loc := #caller_location)-> bool{
    serializer_debug_scope(s, "World_space")
    serialize(s, &world.num_x, loc)or_return
    serialize(s, &world.num_y, loc)or_return
    serialize(s, &world.window_height, loc)or_return
    serialize(s, &world.window_width, loc)or_return
    serialize(s, &world.warp_range, loc)or_return
    serialize(s, &world.y_range, loc)or_return
    serialize(s, &world.start_x, loc)or_return
    serialize(s, &world.start_y, loc)or_return
